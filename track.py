    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                # else:
                #     text = text + str(0)
                # cv2.putText(im0, text, (50, 70), cv2.FONT_HERSHEY_COMPLEX, 3, (250, 250, 0), 8)
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if -1 in de_sum:
                            ...
                            if identities not in de_sum:
                                de_sum.append(identities)
                        #im0 = draw_boxes(im0, bboxes, identities)
                        

                        #count_obj(bboxes,w,h,identities)
                        c = int(cls)  # integer class
                        label = f'{identities} {names[c]} {conf:.2f}'
            ...
            if show_vid:
                ...
                #color=(0,255,0)
                #height, width, channels = im0.shape
                #cv2.line(im0, (0, h // 2), (w, h // 2), (0, 255, 255), 2)

                # color=(0,255,0)
                # start_point2 = (0, h-400)
                # end_point2 = (w, h-400)
...
#     line_count = (0, h // 2)
#     #if int(box[1]+(box[3]-box[1])/2) > (h -300):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='videos/Person1.mp4', help='source')  # file/folder, 0 for webcam
